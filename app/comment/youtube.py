...
from comment.translate import translate
...
def make_comment_from_snippet(obj: dict) -> Comment:
    d = {
        "text": obj.get("snippet", {}).get("textOriginal"),
        "likeCount": obj.get("snippet", {}).get("likeCount"),
...
class YoutubeComment:
    def __init__(self, videoId: str, maxPull=1000):
        youtubeComment = (
            db.query(YoutubeCommentModel)
            .filter(YoutubeCommentModel.videoId == videoId)
            .first()
        )
        if youtubeComment is None:
            data = get_comments(videoId, maxPull)
            youtubeComment = YoutubeCommentModel(
                videoId=videoId,
        ...
        self.comments = [
            Comment.parse_from_json(comment)
            for comment in json.loads(youtubeComment.comments)
        ]

    def get_result(self):
        result = (
            db.query(YoutubeResultModel)
            .filter(YoutubeResultModel.videoId == self.videoId)
            .first()
        )
        if result is None:
